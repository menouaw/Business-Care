...
if (!$isAuthenticated) {
    logSecurityEvent(null, '[SECURITY]:api_access_denied', '[FAILURE] Tentative d\'accès à /api/admin/invoices sans authentification');
    sendJsonResponse(['error' => true, 'message' => 'Authentification requise'], 401);
}

if ($currentUserRole !== ROLE_ADMIN) {
    logSecurityEvent($currentUserId, '[SECURITY]:api_access_denied', '[FAILURE] Utilisateur ID ' . $currentUserId . ' a tenté d\'accéder à /api/admin/invoices sans le rôle admin');
...
if ($method === 'GET') {
    ...
    if ($invoiceId !== null && $invoiceId > 0) {
        ...
        logActivity($currentUserId, '[SECURITY]:api_invoice_request', '[FAILURE] Format d\'ID de facture/entreprise invalide demandé : ' . ($_GET['id'] ?? $_GET['company_id'] ?? 'null'));
        sendJsonResponse(['error' => true, 'message' => 'ID de facture ou d\'entreprise invalide ou non fourni correctement.'], 400);
    }
} else {
    
    logSecurityEvent($currentUserId, '[SECURITY]:api_method_not_allowed', '[FAILURE] Méthode ' . $method . ' tentée sur /api/admin/invoices par l\'Utilisateur ID ' . $currentUserId);
...
function getInvoiceList($companyId = null) {
    ...
    try {
        ...
        logActivity($currentUserId, '[SECURITY]:api_invoice_list', '[SUCCESS] Liste des factures récupérée avec succès (' . count($invoices) . ' factures)' . ($companyId ? ' pour l\'entreprise ID ' . $companyId : ''));
        sendJsonResponse(['error' => false, 'data' => $invoices], 200);

    } catch (Exception $e) {
        logSystemActivity('[SECURITY]:api_invoice_list', '[ERROR] Échec de la récupération de la liste des factures : ' . $e->getMessage());
...
function getInvoiceDetails($id) {
    ...
    try {
        ...
        if ($invoice) {
            ...
            logActivity($currentUserId, '[SECURITY]:api_invoice_detail', '[SUCCESS] Détails récupérés avec succès pour la facture ID : ' . $id);
            sendJsonResponse(['error' => false, 'data' => $invoice], 200);

        } else {
            logActivity($currentUserId, '[SECURITY]:api_invoice_detail', '[FAILURE] Facture non trouvée pour l\'ID : ' . $id);
            sendJsonResponse(['error' => true, 'message' => 'Facture non trouvée'], 404);
        }
    } catch (Exception $e) {
        logSystemActivity('[SECURITY]:api_invoice_detail', '[ERROR] Échec de la récupération des détails pour la facture ID ' . $id . ' : ' . $e->getMessage());
